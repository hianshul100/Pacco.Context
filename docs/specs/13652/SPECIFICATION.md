...
| 2 | esd-generation-internal | 2026-09-25 | The design is strong, but the bookkeeping in the **low-level tier** fails in ways that change what gets built and tested: six of thirteen `E2E-` ids denote different scenarios from §14.A; FR-13 to FR-18 are shifted; BR-1/BR-3 and BR-6/BR-7 are transposed; `C11` stands in for `C7`; the `N1`–`N8` register that `ADR-021` §8 owns and §8.3 traces is renumbered in both waves; a low-level coverage table claims its rows are "carried verbatim" from §14.A and §19 when all eight differ; and a quotation attributed to §10.1 does not appear there. Eight counted obligations from §19 are missing downstream — AC-16's disallowed-origin check, AC-25's machine-caller regression, E2E-13's `N6` replay measurement, AC-26's control inventory, AC-27's observation run, three of FR-12's nine role cases, AC-19's behavioural test, and negative tests for §10.1 invalid transitions 4 and 5. Three premises of this document are contradicted downstream without disclosure: §13.4's out-of-vocabulary-role alert cannot fire under wave-2's telemetry shape, `R-03` is left unmeasured, and a low-level CORS anchor admits an allow-list that BR-8 forbids | 🚫 **No substantive change to this document.** Every finding was a defect in how the low-level tier cites, counts and renumbers identifiers this document owns — not an error in the identifiers themselves. This spec remains the authority for FR-1 to FR-19, BR-1 to BR-8, AC-1 to AC-28, C1 to C12, E2E-1 to E2E-13 and the §12.6 risk register, and for tracing `ADR-021` §8's `N1`–`N8` through §8.3. All fixes were applied in `LOW_LEVEL_SPEC-13652-wave-1.md` and `LOW_LEVEL_SPEC-13652-wave-2.md`: ids realigned to §14.A and §19, the low-level negative anchors renamed to a `NEG-` space so they no longer collide with `N1`–`N8`, wave-local composites moved to an `X-` space, the fabricated §10.1 quotation removed, the eight missing obligations added, and the three contradicted premises closed — a derived `roleRecognised` boolean so §13.4's alert can fire, E2E-13 reinstated as the `R-03` measurement for §12.6 and `B5`, and the CORS anchor tightened to **exactly one** origin so BR-8 holds. This row is appended so the round is on the record in the tier that owns the identifiers | §19.B only | `addressed` |
