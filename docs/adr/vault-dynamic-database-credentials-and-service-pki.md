...
5. ✅ **Every service authenticates to the store with the same static token.** All nine sections use
   ...
   is not reproduced in this record; it is present in each service's settings file, and the file paths
   are given in §6.
...
7. ✅ **Availability is configured as the calling side only.** Its security section carries the header
   name used to transport the certificate and no enable flag
   (`hianshul100_Pacco.Services.Availability/src/Pacco.Services.Availability.Api/appsettings.json:166-170`),
   and its client attaches the certificate under that header
   (`…/Infrastructure/Services/Clients/CustomersServiceClient.cs:17-32`).
...
6. ✅ **The certificate authority is configured but not consumed.** A certificate role and common name
   are requested by nine services, and roles are created for only two of them; meanwhile the services
   that do check certificates read them from disk. Issued certificates are not what is presented.

### 4.3 Neutral / follow-on

1. ✅ Pricing calls Customers over HTTP and is not on the Customers allow list. Either the call does not
   go through the restricted route family, or the allow list is incomplete — this is the platform's
   existing open question on the subject and is carried here.
...
3. **Adopts the pattern's third recommendation as an open question rather than a rule.** The catalog says
   the platform should either consume the issued certificates or stop configuring the authority.
   Consequence 4.2.6 records the current state and question Q2 carries the choice, because the two
   options lead to different services changing.
4. **Constrains `security/edge-authenticated-open-handlers.md`.** That pattern describes authorization
   being absent inside the services (`ADR-006`). The one enabled allow list here is the platform's only
   working example of an internal check, so this record narrows that pattern's scope by one call path
   rather than contradicting it.
5. **Depends on `data/database-per-service.md`.** The credential lease is per service precisely because
   the database is; the two decisions scale together, and a shared database would make rule 1 meaningless.
...
8. **Pattern Update Proposal.** `security/vault-issued-dynamic-credentials-and-service-pki.md` should
   record that two of the eleven deployables are outside the mechanism entirely (consequence 4.2.5) and
   that certificate roles exist for only two services while nine request a certificate role
   (consequence 4.2.6). Its **Related ADRs** entry should change from `None` to `ADR-016`.
...
| 5 | All nine sections use token authentication against the same address with an identical token value (value not reproduced) | ✅ | The nine services' `src/*.Api/appsettings.json` store sections |
...
| 11 | The calling client attaches the certificate under the configured header | ✅ | `hianshul100_Pacco.Services.Availability/src/Pacco.Services.Availability.Infrastructure/Services/Clients/CustomersServiceClient.cs:17,32` |
...
| 15 | Certificate roles are created for two services only, while nine request a certificate role | ✅ | `hianshul100_Pacco/docker-images.txt` compared with evidence 2 |
...
1. **A certificate authority is configured while services load certificates from disk.** Nine services
   request a certificate role and common name from the store (evidence 2), but the two services that
   actually use certificates read them from the filesystem, and roles exist for only two services
   (evidence 15). The configuration describes centrally issued service identity; the code presents
   file-based certificates. The code is authoritative: this record states that the authority is
   configured and not consumed (consequence 4.2.6). Recorded as **Future/Intended State (Not
   Implemented)**.
...
| A2 | The one enabled allow list is the intended model for restricted internal calls, not a leftover experiment. | It is complete and coherent — caller, permission, domain, host — and the calling side is wired to match. | If it is an experiment, rule 4 has no basis and internal calls have no intended identity model at all, which would leave `ADR-006`'s gap unaddressed on every path. |
...
| B2 | Store bootstrap material is committed to the platform repository, so repository access is equivalent to control of the store. | Everyone whose data the platform holds. | **[ACTION NOW]** — recorded as consequence 4.2.2 and evidence 14, cited by path with no value reproduced, so the exposure is visible in this batch and can be scheduled for removal and rotation. |
| B3 | A single shared token is used by all nine services, so the per-service scoping this record describes is not enforced. | Every service, and every database behind one. | **[ACTION NOW]** — stated as decision rule 5 and consequence 4.2.1 rather than left implicit in the configuration's structure. |

### Open Questions

| ID | Question | Why it matters | Owner |
| --- | --- | --- | --- |
| Q1 | Should every service authenticate to the store as itself, and by what method? | Rule 5 requires it. The method matters: a per-service token has the same distribution problem one level down, and the alternatives depend on how the platform is deployed. | **[handled later by the batch 4 authoring stage, in the record covering the deployment path]** |
| Q2 | Should the platform consume the issued service certificates, or stop configuring the authority? | Consuming them makes service identity real across all internal calls; stopping removes configuration that currently implies a guarantee it does not deliver. Both are defensible and they change different services. | **[ACTION NOW]** — both options are stated with their evidence in consequence 4.2.6 and §6.1 item 1, for the deciders assigned under B1 to choose between. |
| Q3 | Should the API gateway and the saga coordinator be brought inside the store mechanism? | They are two of eleven deployables with no central credential source, and the coordinator publishes commands into four services' exchanges with no identity at all (`ADR-011`). | **[handled later by the batch 4 authoring stage, in the record covering service-to-service identity]** |
| Q4 | Should services identify themselves to each other with per-service certificates on every internal call, rather than on one call path? | This is the question `ADR-007` handed to this record. Nine services currently accept internal calls with no service identity, so the allow-list model is a working example rather than a policy. | **[ACTION NOW]** — raised here with the full footprint of the current enforcement, for the deciders assigned under B1 to decide the scope. |
| Q5 | Is Pricing's call to Customers meant to be on the Customers allow list? | If it is, the allow list is incomplete and the call works only because the restricted route family does not cover it. If it is not, the two callers of Customers are governed by different rules for no recorded reason. | **[handled later by the architecture PR review stage]** |
