...
| **Related** | `ADR-018` (the per-repository release rule this extends), `ADR-004` (the edge this surface calls and must not stretch), `ADR-020` (the single-runtime rule this surface falls outside), `ADR-022` (what the surface holds in the browser), `ADR-023` (the origin that makes it reachable) |
...
1. 🎯 **A second toolchain enters the platform.** Every current pipeline pins the same .NET SDK patch
   ...
   The platform gains a runtime whose version nothing currently governs.
...
| `docs/adr/dotnet-core-31-as-platform-runtime-baseline.md` §2 — "Every Pacco deployable targets one runtime version… No service selects its own runtime." | The rule is written for .NET deployables; a browser bundle has no .NET runtime to pin | `ARCHITECTURE_ALIGNMENT_EXCEPTION`. The rule's scope is the .NET runtime of a Pacco service, and this deployable has none. The gap it leaves — nothing governs the surface's Node/toolchain version — is carried as `GAP-13155-07` in `docs/architecture-inventory/risk-constraint-gap-register.md` and as follow-up F4 |
...
| `NFR-23` [maintainability] | A committed frontend build and lint baseline shipped with the surface | §4 obligation 3 — dependency manifest, build and lint/format committed | The repository contains a dependency manifest with a lockfile and a lint/format configuration, and the pipeline runs both |
...
| F4 | Decide what governs the surface's toolchain version, since `ADR-020`'s single-runtime rule does not reach a Node toolchain | Platform owner (unassigned — see B1) | 2026-11-07 | `GAP-13155-07` |
