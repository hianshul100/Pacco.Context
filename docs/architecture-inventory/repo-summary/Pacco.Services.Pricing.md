...
repositories have one; this repository and `Pacco.Services.Vehicles` are the two that do not.
...
5. Why this repository has no `LICENSE` file when eight sibling service repositories do.
...
| Q6 | **[ACTION NOW]** Why does this repository have no `LICENSE` file? | Eight sibling service repositories carry one and this one does not (`Pacco.Services.Vehicles` is the only other omission), which leaves the terms for this code unstated | Add the platform licence, or state deliberately that it differs | Platform owner |
