...
- **`jwt.validateIssuer: false`** in this service, whereas the other nine services set `validateIssuer: true`. Issuer validation is deliberately relaxed on the issuer itself.
...
| Q5 | **[handled later by architecture_evolution_generation]** Why does the token issuer set `validateIssuer: false` when all nine other services set it to `true`? | It is a deliberate-looking inconsistency in the platform's most security-sensitive configuration, and nothing explains it. | Unknown — likely because the service validates its own tokens on `GET /me`. | Security owner |
