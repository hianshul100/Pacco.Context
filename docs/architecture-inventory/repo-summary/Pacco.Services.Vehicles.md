...
repository except `Pacco.Services.Pricing` has one. **Recorded as an observation.**
...
| Q7 | **[ACTION NOW]** Why does this repository have no `LICENSE` file? | Nine sibling service repositories carry one and this one does not, which leaves the terms for this code unstated | Add the platform licence, or state deliberately that it differs | Platform owner |
