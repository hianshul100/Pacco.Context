...
- CI: `.travis.yml` (`dotnet: 3.1.100`, `branches.only: [master, develop]`, `before_script: chmod -R a+x scripts`, `script: ./scripts/build.sh`, `after_success: ./scripts/dockerize.sh`). **No GitHub Actions.**
- **This repository's CI does not run its tests.** `scripts/test.sh` exists here and contains `dotnet test`, but `.travis.yml` never invokes it. Every one of the other ten repositories with a `.travis.yml` — the gateway and the nine other services — runs both `./scripts/build.sh` **and** `./scripts/test.sh`; their Travis files are byte-identical to each other (md5 `e235f59c6c3a954332f0152c8db52df9`) and differ from this one (md5 `2c0d28516e8b33a6c201c1dcac17c572`) **only** by the missing `- ./scripts/test.sh` line. The result is exact and inverted: **the one repository in the platform that has tests is the one repository whose CI does not run them, while eight repositories that contain no test project at all faithfully run `dotnet test` on nothing.** Verified by `grep -o 'script:.*after_success:'` across all eleven files. This resolves what an earlier pass recorded as "whether CI runs the performance tests is Unknown" — CI runs *none* of this repository's five test projects.
- `scripts/dockerize.sh` publishes to **`$DOCKER_USERNAME/pacco.services.availability`**, tagging `latest` + `$TRAVIS_BUILD_NUMBER` on `master` and `dev-$TRAVIS_BUILD_NUMBER` on `develop`. The image organisation is therefore whatever Docker Hub account CI is configured with, not the literal `devmentors` that `Pacco/compose/services.yml` pulls from.
- `scripts/start.sh` runs `export ASPNETCORE_ENVIRONMENT=local; cd src/Pacco.Services.Availability.Api; dotnet run` — **the script uses the correct project path that the README gets wrong** (see "README vs repository").
...
**Documentation-versus-code conflict — resolved this pass:**
- The README carries a Travis badge implying the repository is under continuous integration on a par with its siblings. **The code says otherwise:** `.travis.yml` here omits the `- ./scripts/test.sh` line that all ten sibling repositories carry, so `scripts/build.sh` (`dotnet build -c release`) is the only thing CI runs. The unit, integration, end-to-end, and NBomber performance projects are compiled and never executed. Doc claim: CI-verified. Code reality: build-only. Evidence: `.travis.yml`, `scripts/test.sh`, and the ten sibling `.travis.yml` files.

**Unknown (neither pass yielded proof):**
- Whether the omission of `./scripts/test.sh` was deliberate — for example because the integration and end-to-end suites need MongoDB and RabbitMQ, which the Travis job does not start — or accidental. Nothing in the repository states either.
...
| Q3 | **[ACTION NOW]** Was `- ./scripts/test.sh` deliberately removed from this repository's `.travis.yml`, and if so, what runs these five test projects instead? | This is the platform's only real test suite and its only performance baseline, and CI compiles it without executing a single test. Ten sibling repositories run the step; this one does not. Nothing else in the workspace runs them either. | Unknown. A plausible reason is that the integration and end-to-end suites need MongoDB and RabbitMQ, which the Travis job never starts — but that reasoning is written down nowhere, and it would not explain skipping the unit tests. | Service owner / engineering owner |
...
| Q5 | **[handled later by architecture_evolution_generation]** What do the NBomber performance tests assert, and what would count as a pass? | They are the platform's only performance evidence. Even once they are executed, an unasserted load test produces numbers rather than a gate. | Unknown — the project exists and is compiled by CI, but is never executed by it. | Service owner |
