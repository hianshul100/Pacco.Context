...
| `Pacco.Services.OrderMaker` | `Dockerfile`, `.travis.yml`, `scripts/*`; image `pacco.services.ordermaker`; port 5015. **Deployed by Compose but not by PM2 and not at the edge:** defined in `Pacco/compose/services.yml:78-85` (image `devmentors/pacco.services.ordermaker`, `5015:80`) and `Pacco/compose/services-local.yml:78-85` (`build: ../../Pacco.Services.OrderMaker`), listed under the gateway's `depends_on`, and scraped by `compose/prometheus/prometheus.yml:38-40`. **Absent from the PM2 manifests `Pacco/services.yml` and `Pacco/prod-services.yml`** (10 apps: api, availability, customers, deliveries, identity, operations, orders, parcels, pricing, vehicles) **and from all four `ntrada*.yml` gateway configs**, so it has no edge route | JWT config present but `Program.cs` calls `UseApp()` (not `UseInfrastructure()`); the saga constructs an **empty `CorrelationContext.UserContext`**, i.e. it acts without a caller identity | Seq, Prometheus, Convey logging. **No Jaeger package** — the only broker-participating service without distributed tracing | `Sagas/AIOrderMakingSaga.cs` encodes the whole order-creation choreography; `Chronicle_` 3.2.1 is the saga library choice. **No feature-flag system** | Saga persistence backend; whether this service is deployed at all | No frontend assets — checked `src/**/` (only `certs/`, `Properties/`); no `wwwroot/`, `public/`, `static/`, no `package.json` |
...
Confidence is medium because `ordermaker-service`, although defined in both Compose service stacks
and scraped by Prometheus, is absent from the PM2 process manifests and from every gateway config,
so its `POST /orders` entry point has no caller path visible in the workspace.
...
- `Pacco/README.md` says services can be started "all at once using Docker:
  `docker-compose -f services-local.yml up`". `compose/services-local.yml` exists and does define
  all ten services plus `api-gateway`, including `ordermaker-service`. **No conflict.**
...
| G6 | Which gateway configuration is authoritative per environment | Four configs exist (`ntrada.yml`, `ntrada.docker.yml`, `ntrada-async.yml`, `ntrada-async.docker.yml`) and they differ *architecturally*, not just by hostname: the async pair converts 20 write routes from HTTP `downstream` to RabbitMQ `publish`. `compose/services.yml` sets `NTRADA_CONFIG=ntrada-async.docker.yml`; nothing states the production intent |
...
|---|---|---|---|---|
| `Pacco` | 0 C# projects (`Pacco.sln` references only other repos' projects) + **11 compose-defined deployables** and **10 PM2 apps** (enumerated below) | 0 C# projects; 11 / 11 compose deployables; 10 / 10 PM2 apps | 0 | — (repo documented as a non-code orchestration repository; its deployable checklist is enumerated below rather than deferred) |
...
### 7.1 `Pacco` deployable enumeration

`Pacco` owns no C# project, so `*.csproj` is not its coverage checklist. Its deployables are the
`container_name` entries in the compose service stacks and the `name` entries in the PM2 manifests.
Both lists are enumerated in full below; every entry is documented in §2.3 and `repo-summary/Pacco.md`.

| # | Compose deployable (`compose/services.yml`, same set in `compose/services-local.yml`) | Line | Port | PM2 app (`services.yml`, `prod-services.yml`) |
|---|---|---|---|---|
| 1 | `api-gateway` | 6 | 5000 | `api` |
| 2 | `availability-service` | 17 | 5001 | `availability` |
| 3 | `customers-service` | 26 | 5002 | `customers` |
| 4 | `deliveries-service` | 35 | 5003 | `deliveries` |
| 5 | `identity-service` | 44 | 5004 | `identity` |
| 6 | `operations-service` | 53 | 5005 | `operations` |
| 7 | `orders-service` | 71 | 5006 | `orders` |
| 8 | `ordermaker-service` | 80 | 5015 | **absent from both PM2 manifests** |
| 9 | `parcels-service` | 89 | 5007 | `parcels` |
| 10 | `pricing-service` | 98 | 5008 | `pricing` |
| 11 | `vehicles-service` | 107 | 5009 | `vehicles` |

**11 compose deployables, 10 PM2 apps.** The single difference between the two lists is
`ordermaker-service` (see G2). `compose/infrastructure.yml` additionally defines 10 backing
services (`consul`, `fabio`, `grafana`, `jaeger`, `mongo`, `prometheus`, `rabbitmq`, `redis`,
`seq`, `vault`); these are infrastructure containers, not platform deployables, and are covered in
§3.4.
