...
- **Documentation pass** — root `README.md`, `CONTRIBUTING.md`, `docs/` and `CHANGELOG.md`. Across all thirteen repositories, **only `README.md` ever exists**. There is no `CONTRIBUTING.md`, no `CHANGELOG.md` and no `docs/` directory anywhere in the workspace.
...
| `hianshul100_Pacco.Services.Pricing` | Calculates order price with customer discount | ASP.NET Core 3.1 on Convey; HTTP only | `src/Pacco.Services.Pricing.Api/Program.cs` | **Single project**; domain nested at `src/Pacco.Services.Pricing.Api/Core/` | `customers-service` over HTTP; Consul, Fabio, Jaeger, Prometheus, Seq. **No RabbitMQ, MongoDB, Redis or Vault** |
...
|---|---|---|---|---|---|
| `hianshul100_Pacco` | Vault dev mode, `VAULT_DEV_ROOT_TOKEN_ID=secret`; PKI roles for `availability-service` and `customers-service` with `allowed_domains=pacco.io`; sample unseal keys in `docker-images.txt` | Declares Jaeger, Prometheus, Grafana, Seq | `README.md`, `compose/infrastructure.yml`, `Pacco.sln`, `assets/*.png`. **No feature flags** | Dangling `Pacco.APIGateway.Ocelot` reference; unused relational databases and ELK in `docker-images.txt` | None detected — checked `/`, `assets/`, `compose/`, `scripts/` |
...
Every deployable except `hianshul100_Pacco` and `hianshul100_Pacco.Web` registers with Consul, sits behind Fabio, reports to Prometheus, and logs to Seq. Nine of eleven publish traces to Jaeger; `ordermaker-service` does not, and `hianshul100_Pacco.Web` does not exist. Eight of eleven use Vault; `api-gateway`, `operations-service`, `ordermaker-service` and `pricing-service` do not.
...
Two cases where a service's configuration claims a dependency the code does not exercise:
...
### 5.12 What is documented nowhere at all

The following are architecturally significant, are visible only by reading code, and appear in no README, diagram or comment anywhere in the workspace:
...
- The Chronicle saga and the `Saga` header protocol.
...
- The SignalR hub, its Redis backplane, and the gRPC surface on `operations-service`.
- Vault's dynamic MongoDB credential leasing, used by eight services.
...
- **No infrastructure-as-code beyond Docker Compose.** No Kubernetes manifests, no Helm charts, no Terraform anywhere in the workspace.
...
| Finding | Evidence |
|---|---|
| One JWT signing key literal is checked into three places | `hianshul100_Pacco.APIGateway/src/Pacco.APIGateway/ntrada.yml`, `ntrada-async.yml`; `hianshul100_Pacco.Services.Identity/src/Pacco.Services.Identity.Api/appsettings.json`; `hianshul100_Pacco.Services.Operations/src/Pacco.Services.Operations.Api/appsettings.json` |
...
| `vault.token: "secret"` is committed in eight services | each service's `src/**/appsettings.json` |
| RabbitMQ `guest`/`guest` is committed in nine deployables | each service's `appsettings.json` and the gateway's `ntrada-async.yml` |
| Seq `apiKey: secret` is committed in ten services | each service's `appsettings.json` |
...
- The four PNG diagrams in `hianshul100_Pacco/assets/` were not opened. Any architectural claim they make is unrecorded here.
...
| Q12 | **[handled later by the platform inventory review]** What do the four PNG diagrams in `hianshul100_Pacco/assets/` claim, and do they agree with the code? | They are the platform's only visual documentation and were not opened for this inventory. | Unknown until they are read. | Platform architect |
